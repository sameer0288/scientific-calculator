...
document
  .getElementById("visibility-toggler")
...
trigonoToggle.addEventListener("click", () => {
  document
    .getElementById("trigonometric-container")
...
function bracketFunc(event) {
  ...
  if (
    isNaN(parseFloat(showOnScreen[showOnScreen.length - 1])) == false &&
  ...
  if (calculation.includes("power(")) {
    ...
    if (calculation[calculation.indexOf("power(") - 1] == ")") {
      ...
      for (let i = calculation.indexOf("power(") - 1; i >= 0; i--) {
        ...
        if (bracketCount === 0) {
          break;
        } // if start brackets are equal to end brackets then break this loop
      }
    ...
    // get the index of last number where power function ends
    {
      let bracketCount = 1;
      for (
        let i = calculation.indexOf("power(") + 1;
        i < calculation.length;
        i++
      ) {
        if (calculation[i] == "(") {
          bracketCount++;
        ...
        if (bracketCount == 0) {
          break;
        } // if start brackets are equal to end brackets break this loop
      }
    }
    // calculate power
...
document.addEventListener("keydown", (e) => {
  let targetElement =
    e.key === "Enter"
      ...
      ? delBut
      : !isNaN(e.key)
      ? [...numBut].find((element) => element.innerText === e.key)
      : [...operationBut].find(
          (element) => element.dataset.buttonSymbol === e.key
        );
